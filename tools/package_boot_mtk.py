...
def trailing_fdt(blob: bytes) -> tuple[bytes, int]:
    """Return the exact FDT at the end of the MTK kernel payload."""
    pos = blob.rfind(struct.pack(">I", FDT_MAGIC))
    while pos >= 0:
        if pos + 8 <= len(blob):
            total = struct.unpack_from(">I", blob, pos + 4)[0]
            if total >= 40 and pos + total == len(blob):
                return blob[pos:], pos
        pos = blob.rfind(struct.pack(">I", FDT_MAGIC), 0, pos)
    raise ValueError("no FDT whose totalsize reaches the end of KERNEL_SZ")
...
def validate_baseline(image: bytes, label: str, required_symbol: bytes) -> bytes:
    ...
    dtb, _ = trailing_fdt(kernel_blob(image))
...
def main() -> None:
    ...
    if trailing_fdt(bytes(out[PAGE_SIZE : PAGE_SIZE + len(combined)]))[0] != display_dtb:
